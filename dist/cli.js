...
async function execute(mode, flags) {
    ...
    if (mode.strict) {
        ...
        if (unknowns.fields.length > 0) {
            process.stderr.write(`verify: ${unknowns.fields.length} column(s) exist in the database with no directus_fields row — add a register manifest or a Directus field definition:\n`);
...
async function fetchUnknownFields(client) {
    ...
    for (const row of raw) {
        ...
        // nothing.
        if (rr.meta !== null && rr.meta !== undefined)
            continue;
...
program
    .command("overview")
    .description("Deployment matrix: each target vs its `ref` (from the targets file) across migrations/extensions/config/seeds, plus the promotion queue between refs. Assumes the canonical repo layout (directus_config/, migrations/, extensions/).")
    .option("--targets-file <path>", "path to targets JSON", "./directus-deploy.targets.json")
...
seedsGroup
    .command("pull")
    .description("Regenerate directus_config/seed/<collection>.json from a Directus target, restricted to the seed-managed columns. Preserves an existing file's meta block; parents-before-children ordering for self-referencing collections; fails on duplicate PKs.")
    .argument("<collection>", "collection name (one seed file per invocation)")
    .option("--url <url>", "Directus base URL (env: DIRECTUS_URL)")
    .option("--token <token>", "Directus admin token (env: DIRECTUS_TOKEN)")
    .option("--seed-dir <path>", "path to directus_config/seed", "./directus_config/seed")
    .option("--fields <csv>", "columns to pull (required on first pull; later pulls derive them from the existing file)")
    .option("--filter <json>", "raw Directus filter JSON, passed through to /items")
    .option("--parent-field <name>", "self-referencing FK used for parents-before-children ordering (default: parent_id when pulled)")
    ...
    process.stdout.write(`pulled ${result.rows} row(s) into ${result.path} (fields: ${result.fields.join(",")})\n`);
});
