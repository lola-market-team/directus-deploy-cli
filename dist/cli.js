...
extensionsGroup
    ...
    for (const name of list) {
        try {
            ...
            process.stdout.write(`    build=${r.buildDurationMs}ms rsync=${r.transportDurationMs}ms commit=${r.sourceCommit.slice(0, 8)} ${verify}\n`);
            if (r.verifiedCommit && !r.verifiedCommit.startsWith(r.sourceCommit))
                anyFailed = true;
        }
        catch (e) {
            anyFailed = true;
