...
export async function workspaceDepSrcPaths(repoRoot, extName) {
    const seen = new Set(); // package base names, e.g. "comms"
    async function visit(pkgJsonPath) {
        let pkg;
        try {
...
export async function extensionsByBundledPackage(repoRoot) {
    let exts;
    try {
    ...
    for (const ext of exts) {
        if (!existsSync(join(repoRoot, "extensions", ext, "package.json")))
            continue;
...
async function treeHashesAt(repoRoot, ref, paths) {
    ...
    for (const p of clean)
        out.set(p, await gitTreeHash(repoRoot, ref, p));
    return out;
}
...
async function closureTreeHash(repoRoot, ref, ext, depSrcs) {
    const extSrc = `extensions/${ext}/src`;
    const paths = [extSrc, ...depSrcs];
    const hashes = await treeHashesAt(repoRoot, ref, paths);
    if (!hashes.get(extSrc))
        return null; // extension doesn't exist at this ref
...
async function batchObjectIds(repoRoot, queries) {
    const out = new Map();
    if (queries.length === 0)
        return out;
    const r = await runCommand("git", ["-C", repoRoot, "cat-file", "--batch-check"], { input: queries.join("\n") + "\n" });
...
async function branchHintForTreeHash(repoRoot, ext, treeHash, reference, depSrcs) {
    ...
    const ids = await batchObjectIds(repoRoot, branches.flatMap((b) => paths.map((p) => `${b}:${p}`)));
    ...
    for (const ref of branches) {
        if (!ids.get(`${ref}:${extSrc}`))
            continue; // extension absent at this ref
...
export async function diffExtensions(input) {
    ...
    const depSrcsCache = new Map();
    const depSrcsFor = async (ext) => {
        if (!depSrcsCache.has(ext))
            depSrcsCache.set(ext, await workspaceDepSrcPaths(input.repoRoot, ext));
