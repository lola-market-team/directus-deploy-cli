...
function targetsFile(args) {
    if (args.targets_file)
        return args.targets_file;
    if (args.repo_root)
        return toDefaultDir(args.repo_root, "directus-deploy.targets.json");
...
async function resolveTargetConnection(name, file) {
    ...
    if (!token)
        throw new Error(`target '${name}': $${tokenEnv} is not set in the MCP server env`);
...
async function handleMigrationsStatus(args) {
    let url;
    let token;
    if (args.target) {
...
async function handleExtensionsStatus(args) {
    if (!args.target)
...
async function handleExtensionsDiff(args) {
...
async function main() {
    const server = new Server({ name: "directus-deploy", version: "0.1.0" }, { capabilities: { tools: {} } });
    server.setRequestHandler(ListToolsRequestSchema, async () => ({
        tools: [
            ...
            },
            {
                ...
                description: "READ-ONLY: which migrations/*.sql are applied on the target vs pending vs mutated. Pass `target` (targets-file name, token from $DIRECTUS_<TARGET>_TOKEN) or url+token.",
                inputSchema: {
                    type: "object",
                    properties: {
            ...
            },
            {
                ...
                description: "READ-ONLY: the deployed sourceCommit + buildTime per extension on a target (from each extension's /_meta endpoint). No token needed.",
                inputSchema: {
                    type: "object",
                    properties: {
            ...
            },
            {
                ...
                description: "READ-ONLY cross-env matrix: compares each extension's deployed source tree hash against a git ref (default origin/develop). Immune to squash-merge SHA orphaning. No token needed.",
                inputSchema: {
                    type: "object",
                    properties: {
        ...
        try {
            ...
            switch (req.params.name) {
                ...
                case "directus_overview":
                    payload = await handleOverview(req.params.arguments ?? {});
                    break;
                case "directus_migrations_status":
                    payload = await handleMigrationsStatus(req.params.arguments ?? {});
                    break;
                case "directus_extensions_status":
                    payload = await handleExtensionsStatus(req.params.arguments);
                    break;
                case "directus_extensions_diff":
                    payload = await handleExtensionsDiff(req.params.arguments ?? {});
                    break;
                default:
...
main().catch((e) => {
    process.stderr.write(`directus-deploy MCP: fatal ${e.message}\n`);
    process.exit(1);
});
