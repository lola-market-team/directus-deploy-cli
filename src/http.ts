...
function toTimeoutErr(url: string, ms: number, method = "GET"): TimeoutError {
  const e = new Error(`timeout after ${ms}ms :: ${method} ${url}`) as TimeoutError;
  e.timeout = true;
  return e;
}
  ...
  async function fetchWithRetry(
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        if (isAbort && !replayableOnTimeout) throw lastErr;
