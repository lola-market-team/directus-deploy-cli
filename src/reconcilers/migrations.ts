...
async function fetchAppliedFilenames(client: DirectusClient): Promise<Set<string> | null> {
  ...
  for (const row of rows) {
    if (row && typeof row === "object" && "filename" in row) {
      set.add(String((row as { filename: unknown }).filename));
    }
  }
  return set;
}
...
export async function reconcileMigrations(
  ...
  const applied = (await fetchAppliedFilenames(input.client)) ?? new Set<string>();
  ...
  for (const filename of files) {
    ...
    const recorded = await rawQuery(
      input.client,
      `INSERT INTO ${TRACKER_TABLE} (filename) VALUES ('${basename(filename).replace(/'/g, "''")}')`,
    );
    if (!recorded?.success) {
      results.push({
        kind: "migrations",
